...
int PIVOT_SPEED = 380;  // tuned value; scaled down for 50:1 in setup()
...
// ── Heading-hold during ram (gyro yaw correction) ─────────────────────────────
// When pushing an opponent off-center, the contact force creates a yaw torque
// that spins the attacker's treads along the opponent's surface instead of
// biting in perpendicularly. The LSM6 gyro (g.z = yaw rate) detects this
// rotation. We record the heading at ram start and apply a differential
// correction proportional to accumulated yaw error to maintain perpendicular
// contact throughout the push.
//
// GYRO_YAW_KP: proportional gain on yaw error → motor differential.
//   Too high: oscillates. Too low: doesn't correct fast enough.
//   Starting point: 0.08 — tune up if the bot still deflects, down if it hunts.
//
// PROX_STEER_KP: gain on front sensor asymmetry (frontLeft - frontRight)
//   during the push. Steers toward the opponent's mass center when they rotate.
//   Each count of rawDiff ≈ one LED width of asymmetry.
//   Starting point: 18 — tune up for more aggressive tracking during push.
...
void setup() {
  ...
  if (GEAR_RATIO == 75) {
    ...
    PIVOT_SPEED        = 380;   // tuned value for 75:1
    ARC_TIME           = 900;
    ...
    PIVOT_SPEED        = 255;   // 380 × (75/50×1/1.5) ≈ 255 — same physical turn rate as 380 on 75:1
    ARC_TIME           = 600;
    ...
    case search:
    {
      ...
      if (ANY_FRONT)
      {
        ...
        if (commitPending) {
          ...
          // Strong signal — commit now, alignment is not required.
          frontConfirmCount = 0;
          performRam();
        } else if (abs(rawDiff) <= ALIGN_THRESHOLD) {
          // Weak signal, centred — accumulate confirmation.
          if (++frontConfirmCount >= FRONT_COMMIT_LOOPS) {
            frontConfirmCount = 0;
            performRam();
          }
        } else {
          // Weak signal, still off-centre — steer but don't count.
          frontConfirmCount = 0;
        }
