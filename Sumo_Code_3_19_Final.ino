...
// When performRam returns with the opponent still visible, this flag bypasses
// frontConfirmCount and ALIGN_THRESHOLD for one loop so re-commitment is
// instant rather than requiring a full confirmation cycle from scratch.
bool commitPending = false;

int           PIVOT_SPEED      = 380;
unsigned long ARC_TIME         = 900;
int           ARC_INNER_SPEED  = 120;
unsigned long ARC_DURATION_MS  = 400;
unsigned long EDGE_REVERSE_MS  = 200;
...
const unsigned long STALEMATE_WINDOW_MS    = 8000UL;  // 8 seconds
...
const float GYRO_YAW_KP   = 0.08f;
const int   PROX_STEER_KP = 18;
...
#define ANY_TARGET (ANY_FRONT || sideLeft > 0 || sideRight > 0)
...
// Consecutive evade tracking — detects the "parenthesis bounce" pattern.
// Each time an evade completes, consecutiveEvades increments. If the bot
// leaves the edge and does meaningful search/ram, it resets to 0.
// lastEvadeTurnDir stores which way the last turn went (+1 right, -1 left)
// so the next forced-alternate evade can go the opposite direction.
uint8_t consecutiveEvades   = 0;
int     lastEvadeTurnDir    = 1;   // +1 = turned right, -1 = turned left

enum State      { idle, search, evadeRing };
...
// ── safeDelay ────────────────────────────────────────────────────────────────
// Polls line sensors every 2ms (reduced from 5ms). At 400mm/s the bot travels
// 0.8mm between checks — well within the ~20mm line sensor footprint even when
// moving tangentially along the edge. The old 5ms interval allowed up to 2mm
// of undetected travel per cycle, which was enough to miss a tangential crossing
// of the white ring line when the bot was already at the boundary.
...
void performRam() {
  ...
  // Snapshot heading at ram start for gyro yaw-hold correction.
  // g.z is the yaw rate (rotation around vertical axis, deg/s × scale factor).
  // We accumulate the integral of g.z over time to get heading error in raw
  // gyro units — no need to convert to degrees, just maintain relative zero.
  imu.read();
  long yawIntegral  = 0;          // accumulated yaw from ram start (raw gyro units × ms)
  ...
  while (millis() - start < maxMs) {
    ...
    if (edgeDuringRam != 99) {
      ...
      if (ANY_FRONT) {
        // ── Being-pushed edge escape ──────────────────────────────────────
        // The arc escape was the root cause of the one-off ring exit:
        // the arc drove one wheel at 400mm/s forward while already at the
        // boundary. If the bot's heading was even slightly tangential to the
        // edge, the outer wheel carried it along and out of the ring before
        // safeDelay could catch it.
        //
        // The only geometrically guaranteed-safe escape when already at the
        // edge is: short reverse to clear the sensor, then pivot hard inward.
        // The edge sensor direction tells us definitively which way "inward"
        // is — we don't need to know our heading relative to the ring at all.
        //
        //   edgeDuringRam == -1 (left sensor)  → inward = turn RIGHT
        //   edgeDuringRam ==  1 (right sensor) → inward = turn LEFT
        //   edgeDuringRam ==  0 (center)       → turn either way (use random)
        //
        // Step 1: brief reverse to pull the line sensor off the white ring.
        unsigned long escRevStart = millis();
        while (millis() - escRevStart < EDGE_REVERSE_MS) {
          motors.setSpeeds(-380, -380);
          uint16_t chk[3];
          lineSensors.read(chk);
          if (checkRing(chk) == 99) break;
        ...
        // Step 2: hard pivot inward — direction determined by which sensor fired.
        int pivL, pivR;
        if      (edgeDuringRam == -1) { pivL =  EVADE_TURN_SPEED; pivR = -EVADE_TURN_SPEED; }
        else if (edgeDuringRam ==  1) { pivL = -EVADE_TURN_SPEED; pivR =  EVADE_TURN_SPEED; }
        else {
          // Center sensor: pick the side away from the opponent's push angle.
          if (random(0, 2) == 0) { pivL =  EVADE_TURN_SPEED; pivR = -EVADE_TURN_SPEED; }
          else                   { pivL = -EVADE_TURN_SPEED; pivR =  EVADE_TURN_SPEED; }
        }
        motors.setSpeeds(pivL, pivR);
        // Pivot for the base EVADE_TURN_MS — no jitter here, we need full
        // rotation inward. safeDelay protects against hitting another edge
        // mid-pivot (should not happen but defended against).
        unsigned long baseTurnMs = 700UL;
        if (safeDelay(baseTurnMs)) return;
    ...
    // ── Gyro yaw-hold ──────────────────────────────────────────────────────
    // Integrate g.z over elapsed time to accumulate heading error.
    // A positive g.z means the bot is rotating clockwise (yawing right) —
    // correct by slowing the right wheel relative to the left.
    // This counteracts the bot being deflected along the opponent's surface
    // when contact is not perfectly perpendicular.
    imu.read();
    unsigned long now       = millis();
    unsigned long dt        = now - lastGyroTime;
    lastGyroTime            = now;
    yawIntegral            += (long)imu.g.z * (long)dt;
    int gyroCorr            = (int)((float)yawIntegral * GYRO_YAW_KP / 1000.0f);
    // Hard-cap the gyro correction so it can't override the straight-line drive.
    gyroCorr = constrain(gyroCorr, -80, 80);

    // ── Proximity steering during push ────────────────────────────────────
    // Re-read the front sensor to see if the opponent has rotated off-center.
    // If frontLeft > frontRight, the opponent's mass is to our left — steer
    // left (reduce left, increase right) to keep pushing perpendicular.
    // Combined with gyro correction for total differential adjustment.
    readProxSensors();
    int proxDiff  = (int)frontLeft - (int)frontRight;
    int proxCorr  = proxDiff * PROX_STEER_KP;

    // ── Combined correction ────────────────────────────────────────────────
    // Encoder straight-line correction (Kp * encoder_imbalance) keeps the
    // base trajectory straight. Gyro and prox corrections are additive on top.
    // Priority: prox > gyro > encoder, so cap each layer independently.
    float encAdj = Kp * (float)(l - r);
    int totalAdj = (int)encAdj + gyroCorr + proxCorr;

    int leftCmd  = constrain(maxSpeed - totalAdj, minForward, maxCmd);
    int rightCmd = constrain(maxSpeed + totalAdj, minForward, maxCmd);
...
void setup() {
  ...
  lcd.gotoXY(0, 0); lcd.print("Press A");
  buttonA.waitForPress();
  lcd.clear();

  // Initialise sensors — no motion, no calibration yet.
  lineSensors.initThreeSensors();
  prox.initThreeSensors();

  // ── Rule 6.5: mandatory 5-second start delay ─────────────────────────────
  // "The robot shall not begin moving until after five seconds have passed
  //  since the pressing of the button."
  // Calibration runs during the first second of the countdown so the ring
  // threshold is sampled while the bot is still stationary on the mat.
  // autoCalibrateRing() takes ~375ms (25 samples × 15ms), which fits
  // entirely within the first countdown second with time to spare.
  unsigned long countdownStart = millis();
  bool calibrated = false;
  for (int s = 5; s > 0; s--) {
    lcd.clear();
    lcd.gotoXY(0, 0); lcd.print("Start in:");
    lcd.gotoXY(0, 1); lcd.print(s); lcd.print("s  ");

    // Run calibration on the first second so the bot is on the mat and still.
    if (!calibrated) {
      lcd.gotoXY(4, 1); lcd.print("CAL");
      autoCalibrateRing();
      calibrated = true;
    }

    // Hold for the remainder of this second.
    unsigned long secondEnd = countdownStart + (unsigned long)(5 - s + 1) * 1000UL;
    while (millis() < secondEnd) { /* wait */ }
  }
  lcd.clear();
  ...
  switch (currentState) {
    ...
    case evadeRing:
      switch (currentEvadeState) {
        case EVADE_INIT:
        {
          ...
          // ── Determine this evade's turn direction and duration ─────────────
          // After 2+ consecutive evades the bot is in the parenthesis bounce
          // pattern. Force the opposite turn direction and add a duration boost
          // to guarantee a qualitatively different exit angle.
          // Otherwise, apply a ±20% random jitter to EVADE_TURN_MS so no two
          // consecutive evades produce the exact same pivot angle.
          int turnDir;
          unsigned long thisTurnMs;

          if (consecutiveEvades >= 2) {
            // Forced alternate: flip direction from last evade and turn longer
            // to push the bot well away from the chord it's been bouncing on.
            turnDir    = -lastEvadeTurnDir;
            // Add 40-60% extra turn time on top of base EVADE_TURN_MS.
            thisTurnMs = EVADE_TURN_MS + EVADE_TURN_MS / 2
                         + (unsigned long)random(0, EVADE_TURN_MS / 5);
          } else {
            // Normal evade: random ±20% jitter on turn duration breaks the
            // deterministic trajectory that causes the parenthesis loop.
            turnDir    = (evadeDirection == -1) ? 1 : -1;
            long jitter = (long)random(0, (long)(EVADE_TURN_MS / 5))
                          - (long)(EVADE_TURN_MS / 10);
            thisTurnMs = (unsigned long)max(
                           (long)EVADE_TURN_MS + jitter,
                           (long)(EVADE_TURN_MS / 2));
          }

          // Store direction and duration for use in EVADE_BACK transition.
          // Encode into evadeDirection sign — positive = turn right, negative = left.
          // We repurpose evadeDirection here since edge side is no longer needed
          // once EVADE_INIT has run. Store turnDir in lastEvadeTurnDir now.
          lastEvadeTurnDir = turnDir;
          // Temporarily store the computed turn ms in EVADE_TURN_MS so EVADE_BACK
          // can apply it. Restore after the evade in EVADE_FORWARD.
          EVADE_TURN_MS    = thisTurnMs;
          break;
        }

        case EVADE_BACK:
          if (millis() - evadeTimer >= EVADE_BACK_MS) {
            // Use lastEvadeTurnDir (+1 = right = left-fwd/right-back,
            //                       -1 = left = right-fwd/left-back)
            if (lastEvadeTurnDir == 1)
              motors.setSpeeds( EVADE_TURN_SPEED, -EVADE_TURN_SPEED);
            else
              motors.setSpeeds(-EVADE_TURN_SPEED,  EVADE_TURN_SPEED);
            evadeTimer        = millis();
        ...
        case EVADE_FORWARD:
          if (millis() - evadeTimer >= EVADE_FORWARD_MS) {
            ...
            // Restore EVADE_TURN_MS to the gear-ratio base so the next
            // EVADE_INIT can apply fresh jitter from the correct baseline.
            // (setup() values are captured in these variables at boot.)
            EVADE_TURN_MS = 700;
    ...
    case search:
    {
      ...
      if (ANY_FRONT)
      {
        ...
        // ── Commit logic ────────────────────────────────────────────────────
        // Three paths to commit, in priority order:
        //
        // 1. commitPending: set when performRam just returned with a target
        //    still visible. Skip all confirmation — re-engage immediately
        //    so the bot doesn't lose the opponent during a post-ram arc.
        //
        // 2. Strong signal (maxFront >= COMMIT_THRESHOLD): target is close
        //    and clearly detected. Commit regardless of rawDiff alignment.
        //    The steering command already ran above so the bot is turning,
        //    but waiting for alignment at close range just loses the window.
        //    Previously rawDiff outside ALIGN_THRESHOLD would reset
        //    frontConfirmCount to 0 here — that was the primary cause of
        //    losing the commit mid-approach on brief sensor asymmetries.
        //
        // 3. Weak but centred signal: require FRONT_COMMIT_LOOPS consecutive
        //    aligned readings. Alignment gate still applies here because a
        //    far-away target genuinely needs to be centred before committing.
        if (commitPending) {
          commitPending      = false;
          frontConfirmCount  = 0;
          consecutiveEvades  = 0;   // re-engaged — not bouncing
          performRam();
        } else if (maxFront >= COMMIT_THRESHOLD) {
          frontConfirmCount  = 0;
          consecutiveEvades  = 0;
          performRam();
        } else if (abs(rawDiff) <= ALIGN_THRESHOLD) {
        ...
        // If performRam just returned, check whether the opponent is still
        // visible and set commitPending so the next loop re-engages instantly.
        if (currentState == search && ANY_TARGET) {
          commitPending = true;
        }
      }
      ...
      else
      {
        ...
        imuAccumulator     = 0;
        imuSampleCount     = 0;
        // Bot is sweeping freely — not bouncing on the edge. Reset the
        // consecutive evade counter so the next genuine edge hit starts fresh.
        consecutiveEvades  = 0;
        unsigned long now = millis();
